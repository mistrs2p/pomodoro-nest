...
export class TaskService {
  ...
  async create(userId: number, title: string, estimatedPomodoros = 1) {
    ...
    if (!Number.isInteger(estimatedPomodoros) || estimatedPomodoros < 1 || estimatedPomodoros > 20) {
      throw new BadRequestException('Estimated Pomodoros must be between 1 and 20');
    }
    return this.taskRepository.save(this.taskRepository.create({ userId, title: cleanTitle, estimatedPomodoros }));
  }
